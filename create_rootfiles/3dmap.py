...
# ROOTファイルのパスを指定
root_file_path = "../geant/data/input_test.root"
...
with uproot3.open(root_file_path) as file:
    ...
    # pidに基づいて質量を設定
    mass = np.zeros_like(pid, dtype=float)  # 質量を格納する配列を初期化
    mass[pid == 0] = 0.938272  # Proton
    mass[pid == 1] = 0.139570  # Pion
    mass[pid == 2] = 0.493677  # Kaon
    beta = mom / np.sqrt(mom**2 + mass**2)
...
# 散布図を作成
sc = ax.scatter(mom, tof, beta, c=colors, s=1, alpha=0.7)
